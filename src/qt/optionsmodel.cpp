...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNodesAmount"))
        settings.setValue("nAnonymizeNodesAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeNodesAmount = settings.value("nAnonymizeNodesAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeNodesAmount"))
        SoftSetArg("-anonymizeNodesamount", settings.value("nAnonymizeNodesAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeNodesAmount:
            nAnonymizeNodesAmount = value.toInt();
            settings.setValue("nAnonymizeNodesAmount", nAnonymizeNodesAmount);
            emit anonymizeNodesAmountChanged(nAnonymizeNodesAmount);
            break;
        default:
            break;
        }
